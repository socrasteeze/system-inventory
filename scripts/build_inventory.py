...
def build(workspace):
    ...
    for form_name, flds in data["fields"].items():
        for fld in flds:
            ...
            field_rows.append({
                ...
                "FormulaMode": fld.get("formulaMode",""),
                "FilterCondition": ", ".join(dep.get("filter",[])) or fld.get("filter",""),
                "FilterExpr": (fld.get("filterExpr","") or "")[:500],
                "FilterMode": fld.get("filterMode",""),
                "VisibilityCondition": ", ".join(dep.get("visibility",[])) or fld.get("visibility",""),
                "VisibilityExpr": (fld.get("visibilityExpr","") or "")[:500],
                "VisibilityMode": fld.get("visibilityMode",""),
                "ValidationExpr": (fld.get("validationExpr","") or "")[:500],
                "ValidationMode": fld.get("validationMode",""),
    ...
    sheet(wb.create_sheet("Fields"), "Fields · every field on every form",
          [("FieldKey",38,"PK · FormName::FieldName"), ("FormName",30,"FK → Forms"),
           ...
           ("FormulaMode",14,"Advanced (code) or Conditional (rule builder)"),
           ("FilterCondition",28,"Picklist filter depends on"),
           ("FilterExpr",48,"Raw filter condition expression"),
           ("FilterMode",14,"Advanced (code) or Conditional (rule builder)"),
           ("VisibilityCondition",28,"Visible-when depends on"),
           ("VisibilityExpr",48,"Raw visibility condition expression"),
           ("VisibilityMode",14,"Advanced (code) or Conditional (rule builder)"),
           ("ValidationExpr",48,"Raw required/validation condition expression"),
           ("ValidationMode",14,"Advanced (code) or Conditional (rule builder)"),
